...
// is texSample2D(s, (u)); wrapping s as (s) makes `_ww_sampler_name` paste
// `)_ww_sampler` and volumetricsfront fails to preprocess. The Texture2DMS.Load
// form is only used with BACKBUFFER_MS.
...
// Stock rope geometry becomes one instanced strip per original point. Each invocation
// computes only its selected pair of vertices: O(subdivisions) work per rope segment.
// The original shader still owns Bezier, width, color, fog and refraction expressions.
// Namespaces separate stage helpers; both stages read one shared uniform block.
inline bool LowerPointGeometryToVertex(std::span<WPShaderUnit> units, int subdivisions,
                                       std::string& source) {
    if (units.size() != 3 || units[0].stage != ShaderType::VERTEX ||
        units[1].stage != ShaderType::GEOMETRY || units[2].stage != ShaderType::FRAGMENT)
        return false;
    auto                          vertex   = units[0].src;
    auto                          geometry = units[1].src;
    Map<std::string, std::string> uniforms;
    for (const auto& unit : units)
        for (const auto& [name, type] : unit.preprocess_info.uniforms)
            uniforms.try_emplace(name, type);
    const auto uniform_block = EmitCBufferStd140(uniforms);
    if (! uniforms.empty()) {
        if (! ReplaceAll(vertex, uniform_block, "") || ! ReplaceAll(geometry, uniform_block, ""))
            return false;
    }

    std::vector<IODecl> previous, inputs, interface;
    if (! CollectIODecls(
            &units[0].preprocess_info.output, units[1].debug_name, ShaderType::VERTEX, previous) ||
        ! CollectIODecls(
            &units[1].preprocess_info.input, units[1].debug_name, ShaderType::GEOMETRY, inputs) ||
        ! BuildStageInterface(previous,
                              ShaderType::VERTEX,
                              inputs,
                              ShaderType::GEOMETRY,
                              true,
                              ...
                              interface))
        return false;
    if (! ReplaceAll(geometry, EmitVSFSStruct("WW_VSOut", interface, true), "")) return false;
    ReplaceAll(geometry, "WW_VSOut", "ww_vertex::WW_VSOut");

    HlslTokens tokens;
    if (! TokenizeHlslForRewrite(geometry, units[1].debug_name, ShaderType::GEOMETRY, tokens))
        return false;
    std::optional<std::pair<size_t, size_t>> annotation;
    for (size_t i = 0; i + 1 < tokens.tokens.size(); ++i) {
        if (HlslPunctuationIs(geometry, tokens.tokens[i], '[') &&
            HlslTokenText(geometry, tokens.tokens[i + 1]) == "maxvertexcount") {
            const auto end = tokens.matching_delimiter[i];
            if (end == std::numeric_limits<size_t>::max() || annotation) return false;
            annotation = { tokens.tokens[i].begin, tokens.tokens[end].end };
        }
        if (HlslTokenText(geometry, tokens.tokens[i]) == "RestartStrip") return false;
    }
    if (! annotation) return false;
    geometry.erase(annotation->first, annotation->second - annotation->first);

    if (! TokenizeHlslForRewrite(geometry, units[1].debug_name, ShaderType::GEOMETRY, tokens))
        return false;
    // Recognize the stock subdivision loop structurally after preprocessing. Unsupported
    // control flow must fail compilation, never silently drop particle vertices.
    std::optional<std::pair<size_t, size_t>> loop;
    size_t                                   loop_body = 0;
    for (size_t i = 0; i < tokens.tokens.size(); ++i) {
        if (HlslTokenText(geometry, tokens.tokens[i]) != "for") continue;
        if (loop || i + 1 >= tokens.tokens.size() ||
            ! HlslPunctuationIs(geometry, tokens.tokens[i + 1], '('))
            return false;
        const auto close = tokens.matching_delimiter[i + 1];
        if (close + 1 >= tokens.tokens.size() ||
            ! HlslPunctuationIs(geometry, tokens.tokens[close + 1], '{'))
            return false;
        const auto end = tokens.matching_delimiter[close + 1];
        if (end >= tokens.tokens.size()) return false;
        std::vector<std::string_view> header;
        for (size_t h = i + 2; h < close; ++h)
            header.push_back(HlslTokenText(geometry, tokens.tokens[h]));
        if (header.size() != 12 || header[0] != "int" || header[2] != "=" || header[3] != "0" ||
            header[4] != ";" || header[5] != header[1] || header[6] != "<" ||
            header[7] != std::to_string(subdivisions) || header[8] != ";" || header[9] != "+" ||
            header[10] != "+" || header[11] != header[1])
            return false;
        loop      = { tokens.tokens[i].begin, tokens.tokens[end].end };
        loop_body = tokens.tokens[close + 1].end;
    }
    if ((subdivisions != 0) != loop.has_value()) return false;
    if (loop) {
        const auto        end  = 2 + 2 * subdivisions;
        const auto        body = geometry.substr(loop_body, loop->second - loop_body);
        const std::string indexed =
            "if (OUT.target >= 2 && OUT.target < " + std::to_string(end) +
            ") {\nOUT.current = (OUT.target / 2) * 2;\n" +
            // Evaluate t directly; multiplication avoids a repeated-addition loop.
            "subDivCounter = subDivStep * float(OUT.target / 2);\n" + body +
            "\nOUT.current = " + std::to_string(end) + ";\n";
        geometry.replace(loop->first, loop->second - loop->first, indexed);
    }
    ReplaceAll(geometry, "OUT.Append(v);", "OUT.Append(v); if (OUT.current > OUT.target) return;");
    const std::string original =
        "void main_gs(point ww_vertex::WW_VSOut IN[1], inout TriangleStream<WW_PSIn> OUT)";
    const std::string stream = R"(
struct WW_VertexStream {
    uint target;
    uint current;
    WW_PSIn value;
    void Append(WW_PSIn vertex) {
        if (current == target) value = vertex;
        ++current;
    }
};
void main_gs(ww_vertex::WW_VSOut IN[1], inout WW_VertexStream OUT))";
    if (! ReplaceAll(geometry, original, stream)) return false;
    source = uniform_block + "\nnamespace ww_vertex {\n" + vertex +
             "\n}\nnamespace ww_geometry {\n" + geometry + R"(
}
ww_geometry::WW_PSIn main_vs(ww_vertex::WW_VSIn input, uint vertex : SV_VertexID) {
    ww_vertex::WW_VSOut points[1];
    points[0] = ww_vertex::main_vs(input);
    ww_geometry::WW_VertexStream stream = (ww_geometry::WW_VertexStream)0;
    stream.target = vertex;
    ww_geometry::main_gs(points, stream);
    return stream.value;
}
)";
    return true;
...
bool WPShaderParser::CompileToSpv(std::string_view scene_id, std::span<WPShaderUnit> units,
    ...
    auto compile = [shader_info](std::span<WPShaderUnit> units, std::vector<ShaderCode>& codes) {
        ...
        if (shader_info->emulateGeometry) {
            std::string lowered;
            const auto  found = shader_info->combos.find("TRAILSUBDIVISION");
            const int   subdivisions =
                found == shader_info->combos.end() ? 0 : std::stoi(found->second);
            if (! LowerPointGeometryToVertex(units, subdivisions, lowered)) {
                LOG_ERROR("Cannot lower the point geometry program: %s",
                          units[1].debug_name.c_str());
                return false;
            }
            vunits[0].src = std::move(lowered);
            vunits.erase(vunits.begin() + 1);
        }
    ...
    shader_info->combos["VIVID_POINT_GEOMETRY_VERTEX"] = shader_info->emulateGeometry ? "1" : "0";
    ...
    if (has_cache_dir) {
        ...
        const std::string cache_key_input =
            ...
            std::string(shader_info->emulateGeometry ? "point-geometry-vertex=v2\n" : "") +
            GenSha1(units);
