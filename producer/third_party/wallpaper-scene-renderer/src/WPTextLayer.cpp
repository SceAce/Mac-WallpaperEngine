...
PangoFontMap* SceneTextFontMap() {
    // Asset fonts are registered with Fontconfig. A platform default (CoreText on
    // macOS) cannot see that registry; use the same backend for layout and registration.
    thread_local std::unique_ptr<PangoFontMap, decltype(&g_object_unref)> map(
        pango_cairo_font_map_new_for_font_type(CAIRO_FONT_TYPE_FT), &g_object_unref);
    if (! map) throw std::runtime_error("Scene text requires the Pango FreeType backend");
    return map.get();
}

PangoLayout* CreateSceneTextLayout(cairo_t* cr) {
    auto* context = pango_font_map_create_context(SceneTextFontMap());
...
bool EnsureAssetFontVisibleToCurrentPangoFontMap(AssetFontCacheState&        state,
    ...
    auto* font_map = SceneTextFontMap();
...
bool GenerateTextLayoutImage(fs::VFS& vfs, wpscene::WPTextObject& object,
    ...
    auto* measure_layout  = CreateSceneTextLayout(measure_cr);
    ...
    auto* cr        = cairo_create(surface);
    auto* layout    = CreateSceneTextLayout(cr);
