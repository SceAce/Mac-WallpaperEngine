...
using SceneErrorCallback = std::function<void(std::string)>;
...
constexpr std::string_view PROPERTY_TEXTURE_RESOLUTION   = "texture-resolution";
...
public:
    explicit SceneWallpaper(SceneErrorCallback on_error = {});
    ~SceneWallpaper();
    bool init();
    bool inited() const;
