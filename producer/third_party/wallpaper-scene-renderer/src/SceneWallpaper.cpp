...
public:
    explicit MainHandler(SceneErrorCallback);
    virtual ~MainHandler();
...
public:
    ...
    bool supportsGeometryShaders() const { return m_supports_geometry.load(); }
...
private:
    ...
    MHANDLER_CMD(DRAW) {
        ...
        if (m_rg) {
            ...
            // Wallpaper Engine scene scripts are allowed to mutate an ancestor of an authored
            // camera layer (the Sonic wallpaper rotates CameraBoneMoveMesh, which owns Camera_0).
            // PrepareFrame() has also completed this frame's puppet and camera-path updates, so
            // rebuild the active perspective camera from the final routed world matrix before
            // camera-dependent uniforms or render passes are prepared. Without this handoff the
            // scene graph reports the new roll while SceneCamera keeps its parse-time view/up
            // vectors, making the authored mouse tilt invisible in the rendered frame.
            m_scene->UpdateActiveCameraLayer();
    ...
    MHANDLER_CMD(INIT_VULKAN) {
        ...
        if (msg->findObject("info", &info)) {
            ...
            if (! m_render->init(*info)) throw std::runtime_error("Vulkan initialization failed");
            if (m_render->inited()) {
                m_supports_geometry.store(m_render->supportsGeometryShaders());
...
private:
    ...
    std::atomic<bool>                        m_supports_geometry { true };
...
SceneWallpaper::SceneWallpaper(SceneErrorCallback on_error)
    : m_main_handler(std::make_shared<MainHandler>(std::move(on_error))) {}
...
void MainHandler::loadScene() {
    ...
    if (! vfs.IsMounted("assets")) {
        ...
        if (! sus) {
            throw std::runtime_error("Cannot mount scene assets: " + m_assets);
        }
    }
    ...
    if (! has_pkg_file || ! vfs.Mount("/assets", fs::WPPkgFs::CreatePkgFs(pkgPath))) {
        ...
        if (! vfs.Mount("/assets", fs::CreatePhysicalFs(pkgDir))) {
            throw std::runtime_error("Cannot mount scene directory: " + pkgDir);
        }
    }
...

    {
        ...
        if (scene_src.empty()) {
            throw std::runtime_error("Missing or empty scene entry: " + pkgEntry);
        }
        scene = m_scene_parser.Parse(scene_id,
                                     ...
                                     *m_sound_manager,
                                     ...
                                     m_render_handler->supportsGeometryShaders());
        if (! scene) throw std::runtime_error("Scene parsing failed");
        if (! scene->materialErrors.empty())
            throw std::runtime_error(scene->materialErrors.front());
...
MainHandler::MainHandler(SceneErrorCallback on_error)
...
void MainHandler::fail(std::string message) {
    if (m_failed.exchange(true)) return;
    m_render_handler->frame_timer.Stop();
    LOG_ERROR("Scene failed: %s", message.c_str());
    if (m_on_error) m_on_error(std::move(message));
}
