    ...
    for (const auto& code : codes) {
        ...
        if (stage == VK_SHADER_STAGE_VERTEX_BIT) {
            ...
            for (auto pinput : inputs) {
                auto& input = *pinput;
                // SPIR-V builtins (VertexIndex, InstanceIndex, ...) have no vertex
                // buffer location and DXC may omit their OpName entirely.
                if (input.decoration_flags & SPV_REFLECT_DECORATION_BUILT_IN) continue;
                if (! input.name) {
                    LOG_ERROR("vertex attribute has no reflected name");
                    return false;
                }
