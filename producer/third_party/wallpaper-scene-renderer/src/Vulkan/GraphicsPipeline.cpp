...
GraphicsPipeline& GraphicsPipeline::setTopology(VkPrimitiveTopology topology) {
    ...
    // Strip meshes are non-indexed in this renderer. Restart cannot alter their
    // vertices; Metal requires it enabled for strip topology. Indexed meshes use lists.
    m_input_assembly.primitiveRestartEnable = topology == VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP ||
...
bool GraphicsPipeline::create(const Device& device, vvk::RenderPass& pass,
    ...
    for (const auto& [stage, spv] : m_stage_spv_map) {
        if (!spv) continue;
        if (stage == VK_SHADER_STAGE_GEOMETRY_BIT && ! device.features().geometryShader) {
            LOG_ERROR("material '%s' requires an unsupported geometry shader", debug_name);
            return false;
        }
