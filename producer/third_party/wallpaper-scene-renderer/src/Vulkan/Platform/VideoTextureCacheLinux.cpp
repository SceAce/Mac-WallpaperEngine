#include "VideoTextureCacheLinux.hpp"
...
const char* DecoderRouteName(VideoTextureDecoderRoute route) {
    ...
    case VideoTextureDecoderRoute::Native: return "native";
    }
    return "nvidia";
}
...
struct LinuxVideoTextureCache::Entry {
    std::string key;
...
LinuxVideoTextureCache::LinuxVideoTextureCache(const Device& device, VideoTextureDecoderSettings settings)
    : m_device(device), m_settings(settings) {
    if (! gst_is_initialized()) gst_init(nullptr, nullptr);
    ConfigureDecoderRanks(MakeDecoderSettings(m_settings));
}

LinuxVideoTextureCache::~LinuxVideoTextureCache() = default;

LinuxVideoTextureCache::Entry* LinuxVideoTextureCache::find(std::string_view key) {
    const auto it =
...
const LinuxVideoTextureCache::Entry* LinuxVideoTextureCache::find(std::string_view key) const {
    const auto it =
        std::find_if(m_entries.begin(), m_entries.end(), [key](const auto& entry) {
            return entry != nullptr && entry->key == key;
        });
...
void LinuxVideoTextureCache::allocateCmd() {
...
void LinuxVideoTextureCache::stopPipeline(Entry& entry) {
...
bool LinuxVideoTextureCache::startPipeline(Entry& entry) {
    stopPipeline(entry);
...
bool LinuxVideoTextureCache::restartPipeline(Entry& entry) {
...
bool LinuxVideoTextureCache::loopPipeline(Entry& entry) {
...
bool LinuxVideoTextureCache::applyPipelinePlaybackState(Entry& entry) {
...
bool LinuxVideoTextureCache::setPaused(Entry& entry, bool paused) {
...
bool LinuxVideoTextureCache::stopPlayback(Entry& entry) {
...
bool LinuxVideoTextureCache::setPlaybackRate(Entry& entry, double rate) {
...
bool LinuxVideoTextureCache::seekTo(Entry& entry, double seconds) {
...
bool LinuxVideoTextureCache::uploadSample(LinuxVideoTextureCache::Entry& entry, ::GstSample* sample) {
...
ImageSlotsRef LinuxVideoTextureCache::Acquire(std::string_view key,
...
void LinuxVideoTextureCache::ApplyPlaybackStates(
...
void LinuxVideoTextureCache::SetGlobalPaused(bool paused) {
...
void LinuxVideoTextureCache::ApplySeekRequests(std::unordered_map<std::string, double>& seek_seconds_by_key) {
...
void LinuxVideoTextureCache::Poll() {
    for (auto& entry_ptr : m_entries) {
        auto& entry = *entry_ptr;
...
void LinuxVideoTextureCache::PublishRuntimeStates(
...
void LinuxVideoTextureCache::RecordUploads(vvk::CommandBuffer& cmd) {
    for (auto& entry_ptr : m_entries) {
        auto& entry = *entry_ptr;
        if (entry.pipeline_failed) continue;
...
void LinuxVideoTextureCache::Clear() {
    m_entries.clear();
}

bool LinuxVideoTextureCache::Release(std::string_view key) {
    const auto iter =
        std::find_if(m_entries.begin(), m_entries.end(), [key](const auto& entry) {
            return entry != nullptr && entry->key == key;
        });
...
std::size_t LinuxVideoTextureCache::GetTrackedBytes() const {
    std::size_t total = 0;
...
std::size_t LinuxVideoTextureCache::GetTrackedEntryCount() const {
    return m_entries.size();
}

std::unique_ptr<VideoTextureCache> wallpaper::vulkan::CreateVideoTextureCache(
    const Device& device, VideoTextureDecoderSettings settings) {
    return std::make_unique<LinuxVideoTextureCache>(device, std::move(settings));
}
