...
LoadMaterial(fs::VFS& vfs, const wpscene::WPMaterial& wpmat, Scene* pScene, SceneNode* pNode,
    ...
    if (geometry_stage != GeometryStagePolicy::Disabled) {
        ...
        const bool native_geometry = pScene == nullptr || pScene->supportsGeometryShaders;
        if (geometry_source_exists && ! native_geometry && wpmat.shader != "genericparticle" &&
            wpmat.shader != "genericropeparticle") {
            pScene->materialErrors.push_back("Unsupported geometry material: " + wpmat.shader);
            return std::nullopt;
        }
        pWPShaderInfo->emulateGeometry =
            geometry_source_exists && ! native_geometry && wpmat.shader == "genericropeparticle";
        if (geometry_source_exists && (native_geometry || pWPShaderInfo->emulateGeometry)) {
            sd_units.push_back(WPShaderUnit {
            ...
            pWPShaderInfo->combos["GS_ENABLED"] = "0";
        }
    }
    sd_units.push_back(WPShaderUnit {
    ...
    uint32_t strip_vertices = 0;
    if (pWPShaderInfo->emulateGeometry) {
        const auto found = pWPShaderInfo->combos.find("TRAILSUBDIVISION");
        const int  subdivisions =
            found == pWPShaderInfo->combos.end() ? 0 : std::stoi(found->second);
        // Match the original geometry stage's maximum of 1024 emitted vertices.
        if (subdivisions < 0 || subdivisions > 510) {
            pScene->materialErrors.push_back("Rope subdivision exceeds supported range 0...510");
            return std::nullopt;
        }
        strip_vertices = 4u + static_cast<uint32_t>(subdivisions) * 2u;
    }
    if (! WPShaderParser::CompileToSpv(
            pScene->scene_id, sd_units, shader->codes, vfs, pWPShaderInfo, texinfos)) {
        pScene->materialErrors.push_back("Shader compilation failed: " + wpmat.shader);
        return std::nullopt;
    }
    ...
    return MaterialLoadResult { .geometry_stage_loaded    = geometry_stage_loaded,
                                .instanced_strip_vertices = strip_vertices };
}
...
std::shared_ptr<Scene> WPSceneParser::Parse(std::string_view scene_id, const std::string& buf,
    ...
    context.scene->supportsGeometryShaders = supports_geometry_shaders;
    for (const auto& obj : wp_objs) {
