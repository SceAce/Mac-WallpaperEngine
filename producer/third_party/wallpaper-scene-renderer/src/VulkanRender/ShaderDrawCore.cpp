...
ShaderDrawRenderState BuildCustomShaderRenderState(
    ...
    // Blending does not prove full attachment coverage. Even an opaque draw may cover only a
    // particle quad, discard fragments, or preserve alpha through its write mask. DONT_CARE
    // would invalidate those untouched pixels/channels (visible as tile-sized holes on Metal).
    // Preserve the initialized target unless its owner explicitly requests a clear below.
...
bool ShaderDrawCore::prepare(Scene& scene, const Device& device, RenderingResources& rr) {
    ...
    std::vector<VkVertexInputAttributeDescription> attr_descriptions;
    {
        ...
        for (uint i = 0; i < mesh.VertexCount(); i++) {
            ...
            VkVertexInputBindingDescription bind_desc {
                ...
                .inputRate = mesh.InstanceVertexCount() ? VK_VERTEX_INPUT_RATE_INSTANCE
    ...
    const auto render_state = BuildCustomShaderRenderState(*mesh.Material(), m_desc);
    {
        ...
        pipeline.addDescriptorSetInfo(spanone { descriptor_info })
            ...
            .setTopology(mesh.InstanceVertexCount() ? VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP
                                                    : ToTopology(mesh))
...
void ShaderDrawCore::execute(const Device& device, RenderingResources& rr) {
    ...
    if (m_desc.immutable_mesh) {
        ...
        if (m_desc.immutable_mesh->has_index) {
            ...
            const auto strip = m_desc.node->Mesh()->InstanceVertexCount();
            cmd.Draw(strip ? strip : m_desc.draw_count, strip ? m_desc.draw_count : 1, 0, 0);
        }
    } else {
