...
# JSON pipe opcodes shared with WebUIService.swift; no Linux wire protocol is used.
CONTROL_GET_STATE = 1
...
REMOTE_IMAGE_USER_AGENT = "VividWebUI/0.1 (macOS)"
        ...
        if request_path == "/api/projects":
            try:
                ...
                if include_state or not requested_library_path:
                    control, state = self._native_state()
                else:
                    control, state = None, {}
    ...
    def _post(self):
        ...
        if request_path == "/api/wallpaper/select":
            try:
                ...
                _control, state = self._native_state()
...
def main():
    parser = argparse.ArgumentParser(description="Vivid macOS WebUI with inherited native control pipes")
    ...
    if args.host != "127.0.0.1":
        parser.error("the panel requires loopback host 127.0.0.1")
